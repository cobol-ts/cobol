...
exports.newlineRecordBoundaryDetector = void 0;
exports.createFixedWidthRecordBoundaryDetector = createFixedWidthRecordBoundaryDetector;
const LINE_FEED = 10;
 ...
 */
exports.newlineRecordBoundaryDetector = {
    nextRecordStart(buffers, _recordStart, searchStart) {
        const lineFeed = findByte(buffers, searchStart, LINE_FEED);
        return lineFeed === -1
            ? -1
            : lineFeed + 1;
    },
    recordEnd(buffers, nextRecordStart) {
        /**
         * nextRecordStart is the first byte following LF.
         */
        const lineFeed = nextRecordStart - 1;
        if (byteAt(buffers, lineFeed) !== LINE_FEED) {
            throw new Error("Cannot determine newline record end: "
                + `byte ${lineFeed} is not LF`);
        }
        const previousByte = lineFeed - 1;
        /**
         * CRLF:
         *
         *     a b c CR LF
         ...
         */
        if (previousByte >= 0
            && byteAt(buffers, previousByte) === CARRIAGE_RETURN) {
            return previousByte
                - 1;
        }
        /**
 ...
 *     recordEnd       = 3
 */
function createFixedWidthRecordBoundaryDetector(recordSize) {
    if (!Number.isInteger(recordSize)
        || recordSize <= 0) {
        throw new Error("Fixed record size must be a positive integer; "
            + `received ${recordSize}`);
    }
    return {
        nextRecordStart(buffers, recordStart, _searchStart) {
            const nextRecordStart = recordStart
                + recordSize;
        ...
        recordEnd(_buffers, nextRecordStart) {
            return nextRecordStart
                - 1;
        }
    };
 ...
 * buffers.
 */
function findByte(buffers, startByte, wanted) {
    let logicalOffset = 0;
    for (const buffer of buffers) {
        const bufferEnd = logicalOffset
            + buffer.length;
        /**
        ...
        const bufferStart = Math.max(0, startByte
            - logicalOffset);
        const found = buffer.indexOf(wanted, bufferStart);
        if (found !== -1) {
 ...
 * buffers.
 */
function byteAt(buffers, offset) {
    ...
    throw new Error(`Cannot read logical byte ${offset}: `
...
//# sourceMappingURL=cursor.record.physical.detectors.js.map